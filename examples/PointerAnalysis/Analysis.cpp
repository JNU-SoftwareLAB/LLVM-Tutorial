...
// Variable
std::set<Variable> Variable::Variables = std::set<Variable>();
...
// ConsBinomial
ConsBinomial::ConsBinomial(Operand* left_operand, Operand* right_operand, OperatorCode op, bool condition){
...
bool ConsBinomial::update(){
	...
	// Token ∈ Variable
	if( !(*left_operand).Type.compare("Token") && !(*right_operand).Type.compare("Variable") && op == OperatorCode::in)	{
		// Type Upcasting.
	...
	// Variable1 ⊆ Variable2
	else if( !(*left_operand).Type.compare("Variable") && !(*right_operand).Type.compare("Variable") && op == OperatorCode::subseteq)	{
		// Type Upcasting.
...
std::string ConsBinomial::toString(){
	return this->left_operand->toString() + "\t" + operandToString(this->op) + "\t" + this->right_operand->toString();
}

// ConsIf
ConsIf::ConsIf(Operand* if_left_operand, Operand* if_right_operand, OperatorCode if_op, Constration* cons, bool condition){
...
bool ConsIf::checkCondition(){
	// Token ∈ Variable
	if( !if_left_operand->Type.compare("Token") && !if_right_operand->Type.compare("Variable") && if_op == OperatorCode::in){
		// Down Casting
	...
	// Variable1 ⊆ Variable2
	else if (!if_left_operand->Type.compare("Variable") && !if_right_operand->Type.compare("Variable") && if_op == OperatorCode::subseteq){
		// Down Casting
...
std::string ConsIf::toString(){
	return "If: " + if_left_operand->toString() + "\t" + operandToString(this->if_op) + "\t" + if_right_operand->toString() + "\n\tThen: " + cons->toString();
}
